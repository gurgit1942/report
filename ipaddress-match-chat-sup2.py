...
def load_subnet_mappings(reference_file):
    """
    Load subnet mappings from a CSV file, treating the first row as a header.
    Expected format: ref-subnet,ref-name
    ...
    with open(reference_file, 'r') as f:
        reader = csv.reader(f)
        header = next(reader, None)  # Read and ignore the header row
        if header:
